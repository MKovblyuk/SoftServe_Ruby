...
module Vybory
  ...
  def delete_observer(*observers)
    ...
    for i in (0..@observers.length - 1)
      @observers.delete_at(i) if observers.include? @observers[i]
    end
  ...
  def notify_observers receiver
    ...
    @observers.each {|e| e.notify receiver}
  end
end
...
class Person
  ...
  def initialize name
    ...
    self.add_observer Observer.new("Observer_1"), Observer.new("Observer_2"), Observer.new("Observer_3")
    @name = name
  end
...
class Observer
  ...
  def notify receiver
    puts "Опрацьовано бюлетень за #{receiver}."
  end
